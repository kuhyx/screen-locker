...
void main() {
  ...
  test('pollForToken retries past a dropped connection', () async {
    var calls = 0;
    final client = MockClient((req) async {
      calls++;
      if (calls == 1) {
      ...
      return http.Response(jsonEncode({'access_token': 'gho_retry'}), 200);
    });

    final token = await authWith(client).pollForToken(_device);
    expect(token, 'gho_retry');
    expect(calls, 2);
  });

  test('pollForToken retries past a malformed response body', () async {
    var calls = 0;
    final client = MockClient((req) async {
      calls++;
      if (calls == 1) return http.Response('not json', 200);
      return http.Response(jsonEncode({'access_token': 'gho_ok2'}), 200);
    });

    final token = await authWith(client).pollForToken(_device);
    expect(token, 'gho_ok2');
    expect(calls, 2);
  });
